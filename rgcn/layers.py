...
class UnionRGCNLayer(nn.Module):
    ...
    def propagate(self, g):
        ...
        # lambda x: self.msg_func(x): 这是一个lambda表达式，用于定义消息函数
        # fn.sum 用于将节点收到的所有消息进行聚合。msg参数表示消息的键名,out参数表示聚合后的结果的键名，通常是用于保存聚合结果的节点特征。
        # self.apply_func: 这是一个用户自定义的应用函数，用于将聚合后的消息应用到节点上，更新节点的特征。
    ...
    def msg_func(self, edges):
        ...
        # 根据edges.data['type']中的索引，在self.rel_emb中选择对应的行，返回一个新的张量。
        edge_type = edges.data['type']
        edge_num = edge_type.shape[0]
        node = edges.src['h'].view(-1, self.out_feat)  # 就是 node embedding
